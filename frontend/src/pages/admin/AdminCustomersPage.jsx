...
function CustomerDetail({ userId, onClose, onChanged }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-start md:items-center justify-center p-3 md:p-6 overflow-y-auto" onClick={onClose}>
      <div className="bg-[#0B0E1A] border border-[#B8C0C8]/25 max-w-3xl w-full" onClick={(e) => e.stopPropagation()} data-testid="customer-detail-modal">
        {!data ? (
          <div className="p-12 text-center text-[#8A8FA8] tracking-[0.3em] uppercase text-xs">Loading customer...</div>
        ) : (
          <>
            <div className="flex items-start justify-between p-6 border-b border-[#B8C0C8]/15">
              <div>
            ...
            <div className="grid grid-cols-3 gap-3 p-6">
              <div className="border border-[#B8C0C8]/15 p-4">
                <div className="text-[10px] tracking-[0.3em] uppercase text-[#B8C0C8]">Orders</div>
                <div className="font-serif-display text-2xl text-[#F5F0E8] mt-1">{data.orders_count}</div>
              </div>
              <div className="border border-[#B8C0C8]/15 p-4">
                <div className="text-[10px] tracking-[0.3em] uppercase text-[#B8C0C8]">Total Spent</div>
                <div className="font-serif-display text-2xl text-[#F5F0E8] mt-1">{formatINR(data.total_spent)}</div>
              </div>
              <div className="border border-[#B8C0C8]/15 p-4">
                <div className="text-[10px] tracking-[0.3em] uppercase text-[#B8C0C8]">Loom Cards</div>
                <div className="font-serif-display text-2xl text-[#B8C0C8] mt-1">{data.loom_balance}</div>
            ...
            {adjustOpen && (
              <div className="mx-6 mb-4 border border-[#B8C0C8]/30 p-4">
                <label className="text-[10px] tracking-[0.3em] uppercase text-[#8A8FA8]">Change (+ add, − deduct)</label>
                <div className="flex items-center gap-2 mt-2">
                  <button onClick={() => setChange(change - 1)} className="px-2 py-2 border border-[#B8C0C8]/30 text-[#B8C0C8]"><Minus size={12} /></button>
                  ...
                  <button onClick={() => setChange(change + 1)} className="px-2 py-2 border border-[#B8C0C8]/30 text-[#B8C0C8]"><Plus size={12} /></button>
                </div>
            ...
            <div className="px-6 pb-6">
              ...
              {data.orders.length === 0 ? (
                ...
                <div className="space-y-2 max-h-[50vh] overflow-y-auto pr-1">
                  {data.orders.map((o) => (
                    <div key={o.id} className="border border-[#B8C0C8]/10 p-3 grid grid-cols-12 gap-2 text-sm">
                      ...
                      <div className="col-span-2 md:col-span-1 text-right text-[10px] tracking-[0.25em] uppercase text-[#B8C0C8]">{o.status.replace("_", " ")}</div>
                    </div>
                  ))}
                </div>
              )}
    ...
    <div data-testid="admin-customers-page" className="page-fade">
      ...
      <div className="mt-8 border border-[#B8C0C8]/15 overflow-x-auto">
        <table className="w-full text-left text-sm min-w-[700px]">
          <thead className="text-[10px] tracking-[0.3em] uppercase text-[#B8C0C8] bg-[#11142A]">
            <tr>
          ...
          <tbody className="text-[#F5F0E8]/85">
            {filtered.map((c) => (
              <tr key={c.user_id} className="border-t border-[#B8C0C8]/10 hover:bg-[#B8C0C8]/5 cursor-pointer" onClick={() => setSelected(c.user_id)} data-testid={`customer-row-${c.user_id}`}>
                ...
                <td className="p-4 text-[#B8C0C8]">{c.loom_balance}</td>
                <td className="p-4 text-right text-[#B8C0C8]"><ChevronRight size={16} /></td>
              </tr>
            ))}
          </tbody>
        </table>
