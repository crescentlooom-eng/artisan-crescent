      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-[#0B0E1A] flex items-center justify-center">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full"
            style={{ background: "radial-gradient(circle, rgba(184,192,200,0.06) 0%, transparent 70%)" }} />
        </div>

        {/* Left — text */}
        <div className="absolute left-8 md:left-16 top-1/2 -translate-y-1/2 z-10 max-w-xs">
          <div className="text-[11px] tracking-[0.4em] uppercase text-[#B8C0C8] mb-4"
          ...
          <h2 className="font-serif-display text-4xl md:text-5xl text-[#F5F0E8] leading-tight"
            ...
            {product.name.split(" ").map((w, i) => (
              <span key={i} className={i === product.name.split(" ").length - 1 ? "italic text-[#B8C0C8]/90" : ""}>{w} </span>
            ))}
          </h2>
        ...
        <div className="absolute right-8 md:right-16 top-1/2 -translate-y-1/2 z-10 flex flex-col gap-4">
          {PRODUCTS.map((p, i) => (
            <div key={i} className="flex items-center gap-3">
              <div className={`text-[10px] tracking-[0.3em] uppercase transition-all duration-300 ${i === productIdx ? "text-[#B8C0C8]" : "text-[#8A8FA8]/40"}`}>
                {String(i + 1).padStart(2, "0")}
              </div>
              <div className={`h-px transition-all duration-300 ${i === productIdx ? "w-8 bg-[#B8C0C8]" : "w-3 bg-[#8A8FA8]/30"}`} />
            </div>
          ))}
        </div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2">
          ...
          <div className="w-px h-8 bg-gradient-to-b from-[#B8C0C8]/60 to-transparent animate-pulse" />
        </div>

        {/* Frame progress bar */}
        <div className="absolute bottom-0 left-0 h-px bg-[#B8C0C8]/20 w-full">
          <div className="h-full bg-[#B8C0C8]/60 transition-all duration-100"
